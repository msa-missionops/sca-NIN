...
| 9 | How should business-user-maintained "tag" tables (`Region`, `Top60`, `Source Plant`, `rec_req_type`, `plant_evaluation`) be supplied to the Python pipeline? | **Both CSV-folder and single-workbook loading are supported.** These five tables are genuine Excel `Table` (ListObject) objects the SME edits directly inside the workbook (confirmed via `Excel.CurrentWorkbook(){[Name="tbl_Tag_Region"]}` etc. in `docs/powerquery_m/`), not files exported/copy-pasted between runs. `load_reference_data_from_workbook(path)` (`reference_data.py`) opens one `.xlsx` via `openpyxl` and reads each named table by name (case-insensitive match), regardless of which worksheet it lives on, so the SME's existing edit-in-Excel workflow needs zero change. `PipelinePaths` now accepts either `reference_data_folder` (existing CSV convention) or `reference_data_workbook` (new) — `load_config` requires exactly one to be set. `run_pipeline` dispatches to whichever loader matches. | Confirmed real table names (mixed casing preserved intentionally): `tbl_Tag_Region`, `tbl_Tag_Top60`, `tbl_tag_sourceplant`, `rec_req_type`, `plant_evaluation`. Regression tests: `tests/test_reference_data.py::test_load_reference_data_from_workbook_reads_named_tables_across_sheets` (+ missing-table error case), `tests/test_config.py` (3 tests for the new config option), and an end-to-end pipeline test using the workbook path: `tests/test_pipeline.py::test_run_pipeline_accepts_reference_data_workbook_instead_of_csv_folder`. (`Table_BOBL`/BOBL is excluded here — see Open Decision #10.) |
| 10 | Is BOBL in scope for now? | **DEFERRED — BOBL processing has been removed from the pipeline for now.** Unlike the other five tag tables, BOBL is not business-user-maintained data pasted into the workbook by convenience — it is pulled from a separate PowerBI export, which doesn't belong in `reference_data/` alongside genuinely user-maintained tables. `assemble_nin_base_table` no longer takes a `bobl_enriched` argument; it emits `Backorder Actual`, `Backorder Qnty`, `Backlog Actual`, `Backlog Qnty` as null placeholders so the output schema stays reconciliation-compatible. `nin_pipeline.sources.bobl` (`clean_bobl`/`enrich_bobl`) is untouched and still fully unit-tested — it can be re-wired into `run_pipeline` once the real BOBL input source/refresh mechanism is decided. | User: "we can't place bobl.csv in this directory [reference_data/]. it is actually data that we pull from another source. And in fact we might need to drop this business requirement all together for now." |
| 11 | How should the pipeline run for *all* plants, not just the single plant in `plant_evaluation.csv`? | **New optional `paths.active_plants_folder` config setting.** Points at a folder containing a headerless, single-column CSV of plant codes (one per row, e.g. `USD1`/`MXD1`/`DED2`) — the real file lives at `\\filer3\power_bi$\GSP\Stockout_Excel\Input`. The latest file in that folder is read (same "latest file" convention as every other source folder) via `nin_pipeline.reference_data.load_active_plants`. When set, `run_pipeline` runs the full per-plant pipeline (`_run_for_plant` in `pipeline.py`) once for each listed plant and concatenates every plant's base table into **one combined `nin_base_table`** (not separate per-plant outputs). When unset, the existing single-plant behavior (`plant_evaluation.csv`) is unchanged — fully backward compatible. The run manifest's `sources`/`row_counts` are now nested under a `plants` key (one entry per plant) plus the combined `nin_base_table` row count. | User: "I would like to have this program running for all plants... I am thinking we can control the plant evaluation based on values in the folder \\filer3\power_bi$\GSP\Stockout_Excel\Input." Format confirmed by user: "one plant per row, only one (the first) column. No header used." Output confirmed by user: one combined base table, not separate per-plant runs. Regression tests: `tests/test_reference_data.py::test_load_active_plants_reads_headerless_single_column_csv` (+ trim/upper/dedupe case), `tests/test_pipeline.py::test_run_pipeline_runs_for_every_plant_in_active_plants_folder`. |
| 12 | What file extension do the real live MRP_ELEMENTS_DOH/MRP_ELEMENTS_REC SAP exports use? | **Configurable — new `file_selection.mrp_doh_file_extension`/`mrp_rec_file_extension` settings (default `txt`, unchanged).** A real end-to-end test run against fresh SAP extracts (Aug 2026, `\\filer3\power_bi$\GSP\Stockout_Excel\Output`) showed the live files are written as `.csv`, not `.txt`. `locate_latest_doh_file`/`locate_latest_rec_file` already accepted an `extension` parameter (mirroring the Power Query source's `FileExt` parameter), but `run_pipeline` never exposed it as config — it always used the `"txt"` default. `_run_for_plant` now threads `config.file_selection.mrp_doh_file_extension`/`mrp_rec_file_extension` through to both locate functions. | Same test run also surfaced a copy-artifact discovered while validating "latest file" selection: copying files off the network share resets each run-folder's directory `mtime` to copy-time, not the original write time — which broke `find_latest_run_folder`'s mtime-based ordering for two of four run-folders in the local test copy (`tests/live_input_v2/`, gitignored). This is a **local-test-copy artifact only**, not a pipeline bug: reading directly from the live network share preserves real mtimes. No code change needed for this; documented here in case it recurs when validating against copied sample data. Regression test: `tests/test_pipeline.py::test_run_pipeline_respects_configured_mrp_file_extension`. |
...
**Deferred (see Open Decision #10) — not currently wired into `run_pipeline`.**
The transformation logic below is fully implemented and unit-tested in
`nin_pipeline.sources.bobl`, but it is not called by the pipeline; the
final base table emits its four output columns as null placeholders
instead. This is because, unlike the other reference/tag tables, BOBL is
pulled from a separate PowerBI export rather than being business-user-
maintained data that belongs in `reference_data/`.
...
| Backorder Actual | number | yes | BOBL join | **Always null** — BOBL processing deferred, see Open Decision #10 |
| Backorder Qnty | number | yes | BOBL join | **Always null** — BOBL processing deferred, see Open Decision #10 |
| Backlog Actual | number | yes | BOBL join | **Always null** — BOBL processing deferred, see Open Decision #10 |
| Backlog Qnty | number | yes | BOBL join | **Always null** — BOBL processing deferred, see Open Decision #10 |
