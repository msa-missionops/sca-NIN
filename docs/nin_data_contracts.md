...
| 7 | Is the true final Excel-facing output identical to `Out_Overview_p2_review.csv` (`nin_base_table`'s current schema)? | **No — the real final output has more columns.** `docs/design_reference/output headers.csv` (confirmed against the `output Excel part 1/2/3.png` screenshots) shows the true final table appends `Total Forecast (Qty)` and `week 1` through `week 27` after `Backlog Qnty` — a weekly forecast breakdown that is not in `Out_Overview_p2_review.csv` or the current `nin_base_table`. | This means REC's weekly forecast output (currently excluded from the final table per Open Decision #1) is joined in at a *later* stage than `build_overview_p2_review` — there must be one more downstream Excel/Power Query step (not yet located in `docs/powerquery_m/`) that pivots REC by week and appends it. **Action needed:** locate that additional query/workbook step before Phase 1G reconciliation can be considered complete; Open Decision #1 should be revisited once found. |
