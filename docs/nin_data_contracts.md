...
| 5 | What is MB5T's true header text for the columns after the first `Quantity`/`Crcy` (`Amount in LC`/`Quantity_1`/`Crcy_2` per `stg_mb5t_clean.pq`)? | **Unresolved — flagged loudly, not guessed.** `clean_mb5t`/`parse_pipe_delimited_sap_export` now raise a `ValueError` naming the duplicate header(s) instead of silently colliding. | A real captured production MB5T export (`runs/20260804_143223/raw/MB5T_20260804_085224.csv`) has the header row literally reading `Amount LC`, `Quantity`, `Crcy`, `Quantity`, `Net Value`, `Crcy` (no `_1`/`_2` suffixes, and "in" dropped from `Amount LC`) — likely because SAP truncates the header display text to the same fixed column width as the data. Since Power Query's actual duplicate-header disambiguation behavior for this case is not confirmed, and guessing wrong would silently corrupt the `Quantity`/`Crcy` values used downstream, this needs the real Power Query output or an SME's confirmation of the true field mapping before `clean_mb5t` can safely process this file shape. |
| 6 | Is a duplicate `plant_material_key` possible in PRDPL3? | **Not filtered/deduplicated** — surfaced via `nin_pipeline.validation.reconciliation`'s duplicate-key check instead of silently dropped. | A real captured PRDPL3 export (`runs/20260804_143223/raw/PRDPL3_20260804_084531.csv`) has two rows for `USD1-A2237-04` after cleaning/filtering. The cause (e.g. two source rows differing only in a column not carried into the clean output) has not been root-caused; Phase 1E's downstream joins should be reconciled against production before deciding whether to dedupe. |
