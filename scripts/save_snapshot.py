...
def main():
    ...
    parser.add_argument(
        "--powerquery",
        help="Power Query/current export to save into test_data/expected",
    )
    parser.add_argument(
        "--copy-all-dir", help="Copy all files from a directory into test_data/raw"
    )
    parser.add_argument(
        "--as-of-date", help="As-of date to record in manifest (YYYY-MM-DD)"
    )
    ...
    if args.copy_all_dir:
        ...
        copied["powerquery_output"] = copy_if_provided(
            args.powerquery, os.path.join(ROOT, "test_data", "expected")
        )
